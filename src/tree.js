(function() {
  ...
  let currentTreeData = structuredClone(initialTreeData); // Start with a copy of the initial data
  ...
  const shuffledTreeData = structuredClone(currentTreeData); // Use structuredClone for deep copy
  ...
  update(currentTreeData); // Redraw with the current data
  }
  }
  }
  ...
  currentTreeData = structuredClone(initialTreeData);

  // Reorder the tree based on the selected profession
  if (profession === "animator") {
  // Move "Art" to the top
  const artIndex = currentTreeData.children.findIndex(child => child.name === "Art");
  if (artIndex > 0) {
  const art = currentTreeData.children.splice(artIndex, 1)[0];
  currentTreeData.children.unshift(art);
  }
  } else if (profession === "musicComposer") {
  // Move "Music" to the top
  const musicIndex = currentTreeData.children.findIndex(child => child.name === "Music");
  if (musicIndex > 0) {
  const music = currentTreeData.children.splice(musicIndex, 1)[0];
  currentTreeData.children.unshift(music);
  }
  }

  // Update the visualization with the reordered data
  update(currentTreeData);
  }
  ...
  professionSelect.addEventListener("change", updateTreeData);

  // Initialize the tree
  updateTreeData(); // Initial drawing based on default selection
  })();

(function() {
  ...
  .attr("x2", d => d.target.x) // Corrected: Use target's x
  .attr("y2", d => d.target.y); // Corrected: Use target's y
  ...
  d.fx = event.x; // Keep the node fixed at the dragged position
  d.fy = event.y; // Keep the node fixed at the dragged position
  }
  })();
